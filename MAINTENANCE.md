...
> (CZID-212).** Decisive evidence: `renovate.json` sets `pinDigests: true`, yet every
...
| A4 | **`awscli` pin in `requirements.txt`** | `requirements.txt` (`awscli==…`, with rationale comment) | App shells out to the `aws` CLI **binary**; aegea 4.x dropped the transitive dep, so this must stay explicitly declared or the binary vanishes from prod + CI (CZID-146/151). Renovate *bumps* it, but a human must never delete the line | Keep the explicit pin; bump deliberately, never remove |
| A5 | **Vendored AWS SDK tarballs (forked dep)** | `vendor/aws-sdk-js-v3/*.tgz`; referenced in `package.json` (`"file:vendor/aws-sdk-js-v3/…"`); `Dockerfile:55` copies them | `file:` deps are local tarballs, not registry packages — Renovate npm can't see them | Rebuild/replace the tarball + update the `file:` path + the `Dockerfile:55` COPY by hand |
| A6 | **Forked GitHub Action (moving-tag SSOT)** | `.github/workflows/check.yml` → `uses: thorvath-slower/flake8-action@v2` | The `v2` moving tag in the fork is itself the SSOT; Renovate won't manage a self-owned moving tag | Move the `v2` tag in the fork repo (CZID-204); no edit needed here |
...
| B3 | **npm packages** | `package.json`, `package-lock.json` (registry deps only — *not* the `file:` tarballs, A5) | Renovate `npm` manager (group "npm packages") |
