# frozen_string_literal: true

# SMP-1854 -- pre-account export-control SIGNUP flow. This is a self-service registration that DOES NOT
# exist today: accounts are currently created manually by an admin (users#create), and users#register is
# only a React confirmation page. These four dependency-free, brand-tokened pages (no React/webpack entry)
# work with JS off:
#
#   GET  export_control_signup          -> new      (the registration form)
#   POST export_control_signup          -> create   (validate terms, hand to the async screen, -> pending)
#   GET  export_control_signup/pending  -> pending  (submitted, under review -- the ONLY page an applicant
#                                                     lands on; approved/denied outcomes both arrive by email)
#
# There is deliberately NO approved or denied page: the screen is ASYNC (the ingest returns 202 and the
# verdict arrives later by callback), so at submit time nobody knows the outcome -- everyone lands on
# pending, and ProvisionScreenedAccountJob emails the activation link (approved) or the denial
# (UserMailer.account_creation_denied). Neither outcome routes the accountless applicant back to a page.
#
# PUBLIC / ANONYMOUS: applicants have NO account and NO session, so authenticate_user! is skipped. The
# three export-control gates already return early for a nil current_user, and this controller is ALSO added
# to each gate's exempt list (belt-and-suspenders: a future gate change that assumed a session must never
# trap an applicant in a redirect loop on a page they cannot authenticate past).
#
# create hands the form off to ExportControl::SignupScreeningProducer, which posts the pre-account payload
# to the async screening ingest. FAIL-CLOSED: the producer never provisions (that happens only via the
# service's signed callback), and unconfigured / transport-error / 202 ALL land the applicant on pending.
class ExportControlSignupsController < ApplicationController
  skip_before_action :authenticate_user!
  before_action :disable_header_navigation

  # ISO 3166-1 (English short name + alpha-2 code). HARDCODED on purpose: the app has no countries gem, and
  # the existing profile-form country field is a client-side World Bank API typeahead (JS-dependent) that
  # cannot work on a no-JS page. The option VALUE is the alpha-2 code because that is exactly what the app
  # already sends to Descartes `scountry` (from CloudFront-Viewer-Country) and what the sanctioned-
  # jurisdiction embargo list uses (config/export_control/blocked-jurisdictions.json, alpha-2). A native
  # select therefore removes the "USA vs United States vs us" ambiguity that a free-text field reintroduces.
  COUNTRIES = [
    ["Afghanistan", "AF"], ["Aland Islands", "AX"], ["Albania", "AL"], ["Algeria", "DZ"],
    ["American Samoa", "AS"], ["Andorra", "AD"], ["Angola", "AO"], ["Anguilla", "AI"],
    ["Antarctica", "AQ"], ["Antigua and Barbuda", "AG"], ["Argentina", "AR"], ["Armenia", "AM"],
    ["Aruba", "AW"], ["Australia", "AU"], ["Austria", "AT"], ["Azerbaijan", "AZ"], ["Bahamas", "BS"],
    ["Bahrain", "BH"], ["Bangladesh", "BD"], ["Barbados", "BB"], ["Belarus", "BY"], ["Belgium", "BE"],
    ["Belize", "BZ"], ["Benin", "BJ"], ["Bermuda", "BM"], ["Bhutan", "BT"], ["Bolivia", "BO"],
    ["Bonaire, Sint Eustatius and Saba", "BQ"], ["Bosnia and Herzegovina", "BA"], ["Botswana", "BW"],
    ["Bouvet Island", "BV"], ["Brazil", "BR"], ["British Indian Ocean Territory", "IO"],
    ["Brunei Darussalam", "BN"], ["Bulgaria", "BG"], ["Burkina Faso", "BF"], ["Burundi", "BI"],
    ["Cabo Verde", "CV"], ["Cambodia", "KH"], ["Cameroon", "CM"], ["Canada", "CA"],
    ["Cayman Islands", "KY"], ["Central African Republic", "CF"], ["Chad", "TD"], ["Chile", "CL"],
    ["China", "CN"], ["Christmas Island", "CX"], ["Cocos (Keeling) Islands", "CC"], ["Colombia", "CO"],
    ["Comoros", "KM"], ["Congo", "CG"], ["Congo (Democratic Republic of the)", "CD"],
    ["Cook Islands", "CK"], ["Costa Rica", "CR"], ["Cote d'Ivoire", "CI"], ["Croatia", "HR"],
    ["Cuba", "CU"], ["Curacao", "CW"], ["Cyprus", "CY"], ["Czechia", "CZ"], ["Denmark", "DK"],
    ["Djibouti", "DJ"], ["Dominica", "DM"], ["Dominican Republic", "DO"], ["Ecuador", "EC"],
    ["Egypt", "EG"], ["El Salvador", "SV"], ["Equatorial Guinea", "GQ"], ["Eritrea", "ER"],
    ["Estonia", "EE"], ["Eswatini", "SZ"], ["Ethiopia", "ET"], ["Falkland Islands", "FK"],
    ["Faroe Islands", "FO"], ["Fiji", "FJ"], ["Finland", "FI"], ["France", "FR"],
    ["French Guiana", "GF"], ["French Polynesia", "PF"], ["French Southern Territories", "TF"],
    ["Gabon", "GA"], ["Gambia", "GM"], ["Georgia", "GE"], ["Germany", "DE"], ["Ghana", "GH"],
    ["Gibraltar", "GI"], ["Greece", "GR"], ["Greenland", "GL"], ["Grenada", "GD"],
    ["Guadeloupe", "GP"], ["Guam", "GU"], ["Guatemala", "GT"], ["Guernsey", "GG"], ["Guinea", "GN"],
    ["Guinea-Bissau", "GW"], ["Guyana", "GY"], ["Haiti", "HT"],
    ["Heard Island and McDonald Islands", "HM"], ["Holy See", "VA"], ["Honduras", "HN"],
    ["Hong Kong", "HK"], ["Hungary", "HU"], ["Iceland", "IS"], ["India", "IN"], ["Indonesia", "ID"],
    ["Iran", "IR"], ["Iraq", "IQ"], ["Ireland", "IE"], ["Isle of Man", "IM"], ["Israel", "IL"],
    ["Italy", "IT"], ["Jamaica", "JM"], ["Japan", "JP"], ["Jersey", "JE"], ["Jordan", "JO"],
    ["Kazakhstan", "KZ"], ["Kenya", "KE"], ["Kiribati", "KI"],
    ["Korea (Democratic People's Republic of)", "KP"], ["Korea (Republic of)", "KR"], ["Kuwait", "KW"],
    ["Kyrgyzstan", "KG"], ["Lao People's Democratic Republic", "LA"], ["Latvia", "LV"],
    ["Lebanon", "LB"], ["Lesotho", "LS"], ["Liberia", "LR"], ["Libya", "LY"], ["Liechtenstein", "LI"],
    ["Lithuania", "LT"], ["Luxembourg", "LU"], ["Macao", "MO"], ["Madagascar", "MG"], ["Malawi", "MW"],
    ["Malaysia", "MY"], ["Maldives", "MV"], ["Mali", "ML"], ["Malta", "MT"], ["Marshall Islands", "MH"],
    ["Martinique", "MQ"], ["Mauritania", "MR"], ["Mauritius", "MU"], ["Mayotte", "YT"], ["Mexico", "MX"],
    ["Micronesia (Federated States of)", "FM"], ["Moldova (Republic of)", "MD"], ["Monaco", "MC"],
    ["Mongolia", "MN"], ["Montenegro", "ME"], ["Montserrat", "MS"], ["Morocco", "MA"],
    ["Mozambique", "MZ"], ["Myanmar", "MM"], ["Namibia", "NA"], ["Nauru", "NR"], ["Nepal", "NP"],
    ["Netherlands", "NL"], ["New Caledonia", "NC"], ["New Zealand", "NZ"], ["Nicaragua", "NI"],
    ["Niger", "NE"], ["Nigeria", "NG"], ["Niue", "NU"], ["Norfolk Island", "NF"],
    ["North Macedonia", "MK"], ["Northern Mariana Islands", "MP"], ["Norway", "NO"], ["Oman", "OM"],
    ["Pakistan", "PK"], ["Palau", "PW"], ["Palestine, State of", "PS"], ["Panama", "PA"],
    ["Papua New Guinea", "PG"], ["Paraguay", "PY"], ["Peru", "PE"], ["Philippines", "PH"],
    ["Pitcairn", "PN"], ["Poland", "PL"], ["Portugal", "PT"], ["Puerto Rico", "PR"], ["Qatar", "QA"],
    ["Reunion", "RE"], ["Romania", "RO"], ["Russian Federation", "RU"], ["Rwanda", "RW"],
    ["Saint Barthelemy", "BL"], ["Saint Helena, Ascension and Tristan da Cunha", "SH"],
    ["Saint Kitts and Nevis", "KN"], ["Saint Lucia", "LC"], ["Saint Martin (French part)", "MF"],
    ["Saint Pierre and Miquelon", "PM"], ["Saint Vincent and the Grenadines", "VC"], ["Samoa", "WS"],
    ["San Marino", "SM"], ["Sao Tome and Principe", "ST"], ["Saudi Arabia", "SA"], ["Senegal", "SN"],
    ["Serbia", "RS"], ["Seychelles", "SC"], ["Sierra Leone", "SL"], ["Singapore", "SG"],
    ["Sint Maarten (Dutch part)", "SX"], ["Slovakia", "SK"], ["Slovenia", "SI"],
    ["Solomon Islands", "SB"], ["Somalia", "SO"], ["South Africa", "ZA"],
    ["South Georgia and the South Sandwich Islands", "GS"], ["South Sudan", "SS"], ["Spain", "ES"],
    ["Sri Lanka", "LK"], ["Sudan", "SD"], ["Suriname", "SR"], ["Svalbard and Jan Mayen", "SJ"],
    ["Sweden", "SE"], ["Switzerland", "CH"], ["Syrian Arab Republic", "SY"], ["Taiwan", "TW"],
    ["Tajikistan", "TJ"], ["Tanzania, United Republic of", "TZ"], ["Thailand", "TH"],
    ["Timor-Leste", "TL"], ["Togo", "TG"], ["Tokelau", "TK"], ["Tonga", "TO"],
    ["Trinidad and Tobago", "TT"], ["Tunisia", "TN"], ["Turkey", "TR"], ["Turkmenistan", "TM"],
    ["Turks and Caicos Islands", "TC"], ["Tuvalu", "TV"], ["Uganda", "UG"], ["Ukraine", "UA"],
    ["United Arab Emirates", "AE"], ["United Kingdom", "GB"], ["United States of America", "US"],
    ["United States Minor Outlying Islands", "UM"], ["Uruguay", "UY"], ["Uzbekistan", "UZ"],
    ["Vanuatu", "VU"], ["Venezuela", "VE"], ["Viet Nam", "VN"], ["Virgin Islands (British)", "VG"],
    ["Virgin Islands (U.S.)", "VI"], ["Wallis and Futuna", "WF"], ["Western Sahara", "EH"],
    ["Yemen", "YE"], ["Zambia", "ZM"], ["Zimbabwe", "ZW"],
  ].freeze

  def new
    prepare_form
    render :new
  end

  # Validate the required terms acceptance, then hand the applicant to the async screening producer.
  #
  # Terms acceptance is the ONLY server-side validation, done here (not via native `required`) so the no-JS
  # path is exercised: a submit without the box checked re-renders the form with an accessible error rather
  # than proceeding. On success nothing is persisted here — the producer posts the fields to the async
  # ingest and the applicant lands on pending; provisioning happens only via the service's signed callback.
  def create
    unless params[:terms_accepted] == "1"
      prepare_form
      @terms_error = "You must accept the terms of use and privacy policy to continue."
      return render :new, status: :unprocessable_entity
    end

    # Hand the applicant to the async screening ingest. Every outcome -> pending: the producer never
    # provisions (that happens only via the service's signed callback), and unconfigured / error / 202 are
    # all fail-closed. Log the outcome so a skipped/failed submit is visible rather than looking screened.
    outcome = ExportControl::SignupScreeningProducer.submit(signup_fields)
    Rails.logger.info("[ExportControlSignups] screening submit outcome=#{outcome}")
    redirect_to export_control_signup_pending_path
  end

  def pending
    @show_blank_header = true
    render :pending
  end

  private

  # The fields the producer builds the pre-account screening payload from. Read (not permitted for mass-
  # assignment; nothing is persisted here). Expertise and use case are intentionally NOT collected on this
  # form (see the note in new.html.erb) — the screening payload has no slot for them.
  def signup_fields
    {
      name_first: params[:name_first],
      name_last: params[:name_last],
      email: params[:email],
      institution: params[:institution],
      address1: params[:address1],
      city: params[:city],
      state: params[:state],
      zip: params[:zip],
      country: params[:country],
    }
  end

  def prepare_form
    @show_blank_header = true
    @countries = COUNTRIES
  end
end
