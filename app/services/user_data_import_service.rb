...
class UserDataImportService
  ...
  def import_projects
    each_row("projects") do |row|
      # Projects are ID-PRESERVED and never remapped: the S3 layout embeds
      # project_id (samples/<project_id>/<sample_id>/... for sample inputs/outputs
      # AND workflow-run outputs), and the file transfer copies objects keeping
      # their key -- so importing a project under a new id, or remapping project_id,
      # would orphan every transferred object. We keep the original id.
      #
      # Migration is per-user and trickle (each user self-triggers, any order) while
      # projects are shared, so a project may be encountered by several members over
      # time. Best-effort ownership: the first migrator creates it and becomes its
      # creator (avoids a dangling foreign creator_id); later members just attach;
      # and when the ORIGINAL owner migrates we reassign creator to them. The full
      # creator-id policy + scoping (which projects to migrate, name collisions on a
      # non-fresh target) is under team discussion (see the user-migration
      # project-ownership design doc, shared separately).
      if Project.exists?(row[:id])
        project = Project.find(row[:id])
        add_user_as_member(project)
        # Best-effort: the true owner takes ownership when they migrate.
      ...
      project = insert_one(Project, :projects, {
                             ...
                             # Best-effort ownership: the first user to migrate a project owns it
                             # on the target (avoids a dangling foreign creator_id); reassigned to
                             # the real owner above if/when they migrate. See design doc.
      ...
      (row[:project_workflow_versions] || []).each do |pwv|
        insert_one(ProjectWorkflowVersion, :project_workflow_versions, {
                     ...
                     version_prefix: pwv[:version_prefix],
                   })
      end
